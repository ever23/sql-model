...
class  sqlHelpers
{
    ...
    */
    constructor(config)
    {
        ...
        this.__escapeString=typeof config.escapeString==="function"?config.escapeString:s=>s
    }
    /**
    ...
    */
    __object2boleandSql(valores)
    {
        ...
        for(let i in valores)
        {
            ...
            text+= this.__protectIdentifiers(i.replace(/^(&&|\|\|)/,"")) + "=" + this.__formatVarInsert(valores[i])
        }
        //console.log(text)
        return text
    }
    ...
    */
    filterSqlI(sql,exept=[])
    {
        ...
        {
            let result=this.__escapeString(sql)
            if(/^'.*'$/.test(result))
            {
    ...
    */
    __formatVarInsert(v)
    {
        let result =typeof v==="string"? this.filterSqlI(v):v
        ...
        {
            return "NULL"
        }else if(typeof result==="number" || /^[\d]+(\.[\d]+)?(e\+?[\d]+)?$/i.test(result))
        {
            return result
